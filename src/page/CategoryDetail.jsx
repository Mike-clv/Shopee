  ...
  if (!category) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center">
        ...
        <Link to="/danh-muc"><Button className="mt-4 rounded-full">Xem danh mục khác</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <nav className="mb-6 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
        <Link to="/" className="flex items-center gap-1 hover:text-primary"><Home className="h-3.5 w-3.5" /> Trang chủ</Link>
      ...
      <div className="mb-8">
        <h1 className="mb-2 text-2xl font-bold font-heading sm:text-3xl">Mã Giảm Giá {category.name}</h1>
            ...
            <CardContent className="p-6 sm:p-8">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <h2 className="text-2xl font-bold font-heading">Danh mục {category.name} hiện chưa có mã đang hoạt động</h2>
                  <p className="mt-3 text-sm leading-7 text-muted-foreground">
                    Em giữ đúng dữ liệu sync thật nên khi danh mục này chưa có voucher khả dụng, trang sẽ chuyển sang trạng thái chờ cập nhật thay vì hiển thị số 0 khô cứng. Anh vẫn có thể dẫn người dùng sang các ưu đãi đang chạy ở bên dưới.
                  </p>
                ...
                <div className="rounded-3xl border border-primary/15 bg-primary/5 p-5 text-sm text-muted-foreground lg:max-w-sm">
                  ...
                  <p className="font-semibold text-foreground">Gợi ý để giữ người dùng ở lại</p>
                  <p className="mt-2 leading-7">
                    Thay vì để trang trống, mình ưu tiên dẫn người dùng sang các voucher còn hoạt động để trải nghiệm vẫn liền mạch và chuyên nghiệp.
                  </p>
                </div>
              </div>
            </CardContent>
