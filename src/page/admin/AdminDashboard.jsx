...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { icon: MousePointer, label: 'Tổng lượt nhấn', value: totalClicks, color: 'text-orange-500' },
    { icon: Copy, label: 'Tổng sao chép', value: totalCopies, color: 'text-cyan-500' },
  ...
  return (
    <div className="p-3 sm:p-6">
      <div className="mb-5 sm:mb-6">
        <h1 className="font-heading text-xl font-bold sm:text-2xl">Tổng quan</h1>
      ...
      <Card className="mb-6 sm:mb-8">
        <CardHeader className="pb-2 sm:pb-4">
          <CardTitle className="text-base">Lượt nhấn và sao chép trong 7 ngày gần nhất</CardTitle>
        </CardHeader>
            ...
            <div className="h-[220px] min-w-[540px] sm:h-[250px] sm:min-w-0">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="clicks" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} name="Lượt nhấn" />
                  <Bar dataKey="copies" fill="hsl(var(--accent))" radius={[4, 4, 0, 0]} name="Sao chép" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {topVouchers.map((voucher, index) => (
              <div key={voucher.id} className="flex items-start gap-3 rounded-xl border border-border/70 bg-background/80 p-3 text-sm">
                ...
                <span className="shrink-0 text-right text-xs font-semibold text-muted-foreground sm:text-sm">{voucher.click_count || 0} lượt</span>
              </div>
            ))}
          </div>
        </CardContent>
